...
export async function task(roundNumber) {
  ...
  try {
    ...
    let koii_main_account_pubkey = namespaceWrapper.getMainAccountPubkey();

    // Check to see if the email exists in Pentagon's system
    let queryResponse = await findUserInPentagon(pentagon_games_email);
...
async function findUserInPentagon(userData) {
  try {
      // Create form data
